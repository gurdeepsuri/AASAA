...
function shareInvoice(inv, client) {
  ...
  const st = settings();
  const lines = [
    `${st.businessName || 'AASAA Studio'} — Invoice ${inv.number}`,
...
function printInvoice(inv, client, project) {
  ...
  area.innerHTML = `
    <div class="pq">
      <div class="pq__head">
        <div>
          <div class="pq__brand">${escapeHtml(st.businessName || 'AASAA Studio')}</div>
      ...
      <div class="pq__foot">Thank you for your business. — ${escapeHtml(st.businessName || 'AASAA Studio')}</div>
    </div>`;
